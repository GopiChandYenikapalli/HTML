...
merged_data = import("merged_data_S308C_Flow_NH3.csv")
...
# Rename the columns
colnames(merged_data)[colnames(merged_data) == "S308C_AMMONIA-N_mg/L"] <- "S308C_AMMONIA_N_Ld_mg"
colnames(merged_data)[colnames(merged_data) == "S308.DS_FLOW_cfs"] <- "S308.DS_FLOW_cfd"
# Calculate new column
merged_data$S308C_AMMONIA_N_Ld_mg <- merged_data$S308.DS_FLOW_cfd * merged_data$S308C_AMMONIA_N_Ld_mg* 1000
# Check skewness
skewness_values <- sapply(merged_data[c("S308C_AMMONIA_N_Ld_mg", "S308.DS_FLOW_cfd")], skewness)
print(skewness_values)

# Check kurtosis
kurtosis_values <- sapply(merged_data[c("S308C_AMMONIA_N_Ld_mg", "S308.DS_FLOW_cfd")], kurtosis)
print(kurtosis_values)

# Set the plotting parameters
par(mfrow = c(1, 2))  # To display histograms in a 1x2 grid

# Histogram for S308C_AMMONIA_N_Ld_mg
hist(merged_data$S308C_AMMONIA_N_Ld_mg, col = 'red', probability = TRUE)
curve(dnorm(x, mean = mean(merged_data$S308C_AMMONIA_N_Ld_mg), sd = sd(merged_data$S308C_AMMONIA_N_Ld_mg)), 
      from = min(merged_data$S308C_AMMONIA_N_Ld_mg), to = max(merged_data$S308C_AMMONIA_N_Ld_mg), 
...
model <- nls(S308C_AMMONIA_N_Ld_mg ~ a * S308.DS_FLOW_cfd^b, data = merged_data, start = initial_values)
...
message("Non-linear equation: S308C_AMMONIA_N_Ld_mg =", a, "* S308.DS_FLOW_cfd^", b)
...
plot_data <- data.frame(
  ...
  Actual = merged_data$S308C_AMMONIA_N_Ld_mg,
...
ggplot(plot_data, aes(x = S308.DS_FLOW_cfd)) +
  ...
  labs(x = "S308.DS_FLOW_cfd", y = "S308C_AMMONIA-N_Ld_mg") +
  ggtitle("Actual vs Predicted Values") +
  theme_minimal()

# Calculate the Root Mean Square Error (RMSE)
rmse <- sqrt(mean((merged_data$S308C_AMMONIA_N_Ld_mg - predictions)^2))
message("RMSE:", rmse)
...
mean_actual <- mean(merged_data$S308C_AMMONIA_N_Ld_mg)
sst <- sum((merged_data$S308C_AMMONIA_N_Ld_mg - mean_actual)^2)

# Calculate the residual sum of squares (SSE)
sse <- sum((merged_data$S308C_AMMONIA_N_Ld_mg - predicted)^2)
