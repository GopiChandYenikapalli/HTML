...
merged_data$S135_AMMONIA_N_Ld_mg <- merged_data$S135_C_FLOW_cfd * merged_data$S135_AMMONIA_N_Ld_mg* 1000
# Save updated merged_data as a CSV file
write.csv(merged_data, "merged_data_S135_Flow_NH3_updated.csv", row.names = FALSE)
# Check skewness
skewness_values <- sapply(merged_data[c("S135_AMMONIA_N_Ld_mg", "S135_C_FLOW_cfd")], skewness)
print(skewness_values)

# Check kurtosis
kurtosis_values <- sapply(merged_data[c("S135_AMMONIA_N_Ld_mg", "S135_C_FLOW_cfd")], kurtosis)
...
model <- nls(S135_AMMONIA_N_Ld_mg ~ a * S135_C_FLOW_cfd^b, data = merged_data, start = initial_values)
...
message("Non-linear equation: S135_AMMONIA_N_Ld_mg =", a, "* S135_C_FLOW_cfd^", b)
...
ggplot(plot_data, aes(x = S135_C_FLOW_cfd)) +
  ...
  labs(x = "S135_C_FLOW_cfd", y = "S135_AMMONIA-N_Ld_mg") +
